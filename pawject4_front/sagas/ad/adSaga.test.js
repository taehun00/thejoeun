...
describe('adSaga 테스트', () => {
  ...
  it('fetchAd 성공 시 fetchAdSuccess를 디스패치해야 한다', async () => {    
    ...
    api.get.mockResolvedValue({ data: mockData });  // 서버에서 받아오는 값 - 덤프 / 단 건 조회 → []배열X
    const dispatched = [];

    const action = fetchAdRequest({adId: 1});  // react - view
    await runSaga({dispatch: (a) => dispatched.push(a) }  , fetchAd , action ).toPromise();
    
    expect(dispatched).toContainEqual(fetchAdSuccess(mockData)); //단 건 조회 → []배열X
  ...
  it('createAd 성공 시 createAdSuccess를 디스패치해야 한다', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/ads/ads', expect.any(FormData), {
  ...
  it('updateAd 성공 시 updateAdSuccess를 디스패치해야 한다', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/ads/1/ads', expect.any(FormData), {
      headers: { "Content-Type": "multipart/form-data" }
    });
  });
  ...
  it('deleteAd 성공 시 deleteAdSuccess를 디스패치해야 한다', async () => {
    ...
    // deleteAdSuccess는 payload로 adId를 전달함 (adReducer.js 참고)
