...
-- 새로운 알림( 모든 유저에게 알림을 보냄 )
INSERT INTO NOTIFICATION_M (
    notificationMID, USERID, BOARDTYPE, TYPE, MESSAGE, CREATEDAT
)
SELECT notification_seq.NEXTVAL, u.USERID, '운동정보', 'CREATE',
       '운동정보 게시판에 새로운 정보가 등록됐습니다.', SYSDATE
FROM USERS u;




-- 회원가입 시 알림 기본 셋팅(이므로 시퀀스 필요 x)
-- 운동정보 알림 기본 ON
