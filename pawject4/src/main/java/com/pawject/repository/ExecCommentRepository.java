...
public interface ExecCommentRepository extends JpaRepository<ExecComment, Long> { //Entity , PK ★
	...
	List<ExecComment> findByPost_IdAndDeletedFalse(Long execPost); // Comment 엔티티 post필드 확인, 그 안에 id가 매핑
	
	//삭제되지 않은 댓글 수 집계
	long  countByPost_IdAndDeletedFalse(Long execPost); // Comment 엔티티 post필드 확인, 그 안에 id가 매핑
}
