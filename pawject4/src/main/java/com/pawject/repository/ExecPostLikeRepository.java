...
public interface ExecPostLikeRepository extends JpaRepository<ExecPostLike, Long> { //Entity , PK ★
	...
	long  countByUserIdAndExecPostId(Long userId, Long postId);
	
	// 좋아요 취소 (조회없이 바로 삭제) 
	@Modifying
	@Transactional
	@Query("DELETE FROM PostLike pl  WHERE  pl.user.id = :userId AND  pl.execPost.id= :postId ")
	void deleteByUserAndPost(@Param("userId") Long userId  , @Param("postId") Long  postId);
	
	// 특정유저의 특정게시글 좋아요 조회
	Optional<ExecPostLike> findByUserIdAndExecPostId(Long userId, Long postId);
}
